...
function isRowOk(grid, row, num) {
  for (var col = 0; col < 9; col++) if (grid[row][col] == num) return false

  return true
}
function isColOk(grid, col, num) {
  for (var row = 0; row < 9; row++) if (grid[row][col] == num) return false
...
function isBoxOk(grid, row, col, num) {
  ...
  for (var r = 0; r < 3; r++) for (var c = 0; c < 3; c++) if (grid[row + r][col + c] == num) return false
